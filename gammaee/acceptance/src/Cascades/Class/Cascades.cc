...
Cascades::event( Frame& iFrame )          // anal3 equiv.
{
   ...
   if (!triggerDecision->isTwoTrack()) return ActionBase::kFailed;
   ...
   if (props->firstTrackIdentifier() == 0  ||
       props->secondTrackIdentifier() == 0    ) return ActionBase::kFailed;
   ...
   track_iter = tracks.find(props->firstTrackIdentifier());
   other_iter = tracks.find(props->secondTrackIdentifier());

   if (track_iter->pionHelix()->curvature() *
       other_iter->pionHelix()->curvature() >= 0.) return ActionBase::kFailed;

   if (track_iter->pionFit()->momentum().perp() <= 0.150 ||
       other_iter->pionFit()->momentum().perp() <= 0.150   ) return ActionBase::kFailed;
...
//      m_ntuple->fill(v);
      ...
      if (((v[k_visen] - abs(v[k_tre1]) - abs(v[k_tre2]))*v[k_ebeam]/4.73 > 0.2)  &&
	  ((v[k_ccen] - v[k_she1] - v[k_she2])*v[k_ebeam]/4.73 < 0.9)             &&
	  (v[k_e1drop]*v[k_ebeam]/4.73 < 0.9)                                     &&
	  (v[k_p2drop]*v[k_ebeam]/4.73 < 0.9)                                     &&
	  v[k_mass] > 9.4575                                                      &&
	  v[k_mass] < 9.47                                                          )
      {
	 ...
	 for (track_iter = tracks.begin();  track_iter != tracks_end;  ++track_iter) {
	    report(SYSTEM, kFacilityString)
	       << "track " << track_iter->identifier() << endl
	       << "   fit = " << track_iter->pionQuality()->fit()
	       << " fitAbort = " << track_iter->pionQuality()->fitAbort()
	       << " chi2, dof = " << track_iter->pionQuality()->chiSquare() << ", " << track_iter->pionQuality()->degreesOfFreedom()
	       << " hits, expect = " << track_iter->seedQuality()->numberLayers() << ", " << track_iter->seedQuality()->numberHitsExpected() << endl
	       << "   d0 = " << track_iter->pionHelix()->d0()
	       << " z = " << track_iter->pionHelix()->position().z()
	       << " p/ebeam = " << track_iter->pionFit()->momentum().mag() / eBeam->value()
	       << " cotTheta = " << track_iter->pionHelix()->cotTheta()
	       << " errcT = " << sqrt(fabs(track_iter->pionHelix()->errorMatrix()(KTHelix::kCotTheta, KTHelix::kCotTheta)))
	       << " errZ0 = " << sqrt(fabs(track_iter->pionHelix()->errorMatrix()(KTHelix::kZ0, KTHelix::kZ0))) << endl;
	 }

	 report(SYSTEM, kFacilityString)
	    << "mass = " << v[k_mass] << endl
	    << "p1 = " << v[k_p1x] << " " << v[k_p1y] << " " << v[k_p1z] << endl
	    << "p2 = " << v[k_p2x] << " " << v[k_p2y] << " " << v[k_p2z] << endl
	    << "she1 = " << v[k_she1] << " she2 = " << v[k_she2] << endl
	    << "tre1 = " << v[k_tre1] << " tre2 = " << v[k_tre2] << endl
	    << "tracks = " << v[k_tracks] << " visen = " << v[k_visen] << " ccen = " << v[k_ccen] << endl
	    << "p2 = " << v[k_p2] << " p2drop = " << v[k_p2drop] << " e1 = " << v[k_e1] << " e1drop = " << v[k_e1drop] << endl
	    << "run = " << v[k_run] << " ebeam = " << v[k_ebeam] << " neue = " << v[k_neue] << " q = " << v[k_q] << endl
	    << "hadron = " << v[k_hadron] << " radtau = " << v[k_radtau] << " eltrack = " << v[k_eltrack] << " twotrack = " << v[k_twotrack] << endl
	    << "rawtk = " << v[k_rawtk] << " pmiss = " << v[k_pmiss] << " tr1 = " << v[k_tr1] << " tr2 = " << v[k_tr2] << endl
	    << "trperp1 = " << v[k_trperp1] << " trperp2 = " << v[k_trperp2] << " e2 = " << v[k_e2] << endl;
	 report(SYSTEM, kFacilityString)
	    << "pion1 = " << pion1->identifier() << " pion2 = " << pion2->identifier() << endl;
