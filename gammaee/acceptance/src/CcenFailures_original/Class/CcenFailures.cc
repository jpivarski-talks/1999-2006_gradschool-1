...
CcenFailures::event( Frame& iFrame )          // anal3 equiv.
{
   ...
   extract(iFrame.record(Stream::kEvent), eBeam);
   ...
   if (e1 > 0.85*eBeam->value()) return ActionBase::kFailed;
   if (total < 0.85*2.*eBeam->value()) return ActionBase::kFailed;
   ...
   if (props->secondLargestPTrack() > 0.85*eBeam->value()) return ActionBase::kFailed;
   if ((props->neutralEnergy() + props->chargedEnergy()) < 0.2*2.*eBeam->value()) return ActionBase::kFailed;
