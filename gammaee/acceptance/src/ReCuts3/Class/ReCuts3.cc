...
//
// Author:      Jim Pivarski
// Created:     Fri Aug  6 13:21:05 EDT 2004
// $Id$
//
// Revision history
//
...
//
static const char* const kFacilityString = "Processor.ReCuts3" ;
   ...
   : Processor( "ReCuts3" )
{
   ...
   bind( &ReCuts3::event,    Stream::kEvent );
   //bind( &ReCuts3::beginRun, Stream::kBeginRun );
   //bind( &ReCuts3::endRun,   Stream::kEndRun );
...
// --------------------------- init method -------------------------
void
ReCuts3::init( void )          // anal1 "Interactive"
{
...
// -------------------- terminate method ----------------------------
void
ReCuts3::terminate( void )     // anal5 "Interactive"
{
...
void
ReCuts3::hist_book( HIHistoManager& iHistoManager )
{
...
ReCuts3::event( Frame& iFrame )          // anal3 equiv.
{
   ...
   if (!(p2 < 0.85)) return ActionBase::kFailed;
   if (!(e1 < 0.85)) return ActionBase::kFailed;
//    if (!(visen > 0.2)) return ActionBase::kFailed;
//    if (!(ccen < 0.85)) return ActionBase::kFailed;
//    if (!(ntracks > 1)) return ActionBase::kFailed;

   if (!(visen < 0.3)) return ActionBase::kFailed;
...
/*
ActionBase::ActionResult
ReCuts3::beginRun( Frame& iFrame )       // anal2 equiv.
{
...
/*
ActionBase::ActionResult
ReCuts3::endRun( Frame& iFrame )         // anal4 equiv.
{
...
DABoolean ReCuts3::calc_intersection(double& x, double& y, double& z, double& sig2z, double& zsep,
				      KTHelix& a, KTHelix& b)
{
...
DABoolean ReCuts3::arclength_test(double x, double y, KTHelix& a, KTHelix& b)
{
