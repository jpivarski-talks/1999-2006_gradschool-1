...
// $Id: KaonMonitorProc.cc,v 1.1.1.1 2003/11/18 22:04:18 mccann Exp $
//
// Revision history
//
// $Log: KaonMonitorProc.cc,v $
...
static const char* const kIdString  = "$Id: KaonMonitorProc.cc,v 1.1.1.1 2003/11/18 22:04:18 mccann Exp $";
...
//    static int shortpairs = 0;
//    static int longpairs = 0;
//    static int allevents = 0;
//    int shortcount = 0;
//    int longcount = 0;
//    int elsecount = 0;
//    for (MCVertex::const_pIterator child_iter = decayTree->topParticle().deathVertex()->pBegin()->deathVertex()->pBegin()->deathVertex()->pBegin();
// 	child_iter != decayTree->topParticle().deathVertex()->pBegin()->deathVertex()->pBegin()->deathVertex()->pEnd();
// 	++child_iter) {
//       if (child_iter->properties().name() == "anti-K0") shortcount++;
//       else if (child_iter->properties().name() == "K0") longcount++;
//       else elsecount++;

//       report(DEBUG, kFacilityString) << child_iter->properties().name() << endl;
//    }
//    report(DEBUG, kFacilityString) << endl;
//    if (shortcount == 1  &&  longcount == 1  &&  elsecount == 0) shortpairs++;
//    allevents++;
//    if (allevents % 1000 == 0) 
//       cout << "pairs = " << shortpairs << " allevents = " << allevents << endl;

   static int two_kaon_events = 0;
...
// // FOR THE MCSTUDY
//    static int real_kaons = 0;
//    static int observed_kaons = 0;
//    FAItem<MCDecayTree> decayTree;
//    extract(iFrame.record(Stream::kEvent), decayTree);
//    MCDecayTree::const_vIterator vertex_end = decayTree->vEnd();
...
// 	++vertex_iter) {
//       if (vertex_iter->parent().properties().name() == "K_S0") {
// 	 int piplus = 0;
// 	 int piminus = 0;
...
// END MCSTUDY (FOR NOW)
   ...
   if (observed_kaons > 0  &&  tracks.size() == observed_kaons * 2) two_kaon_events++;
   ...
   if (all_events % 100 == 0)
      cout << "two_kaon_events = " << two_kaon_events << " all_events = " << all_events << endl;
