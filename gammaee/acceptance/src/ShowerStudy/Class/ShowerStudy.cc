...
// user include files
#include "ShowerStudy/ShowerStudy.h"
...
//
static const char* const kFacilityString = "Processor.ShowerStudy" ;
...
//
ShowerStudy::ShowerStudy( void )               // anal1
   : Processor( "ShowerStudy" )
{
   ...
   bind( &ShowerStudy::event,    Stream::kEvent );
   //bind( &ShowerStudy::beginRun, Stream::kBeginRun );
   //bind( &ShowerStudy::endRun,   Stream::kEndRun );
...
// --------------------------- init method -------------------------
void
ShowerStudy::init( void )          // anal1 "Interactive"
{
...
// -------------------- terminate method ----------------------------
void
ShowerStudy::terminate( void )     // anal5 "Interactive"
{
...
void
ShowerStudy::hist_book( HIHistoManager& iHistoManager )
{
...
ShowerStudy::event( Frame& iFrame )          // anal3 equiv.
{
   ...
   report(DEBUG, kFacilityString)
      << "p2 " << p2 << " e1 " << e1 << " visen " << visen << endl;
   if (p2 > 0.85  ||  e1 > 0.85  ||  visen > 0.35) return ActionBase::kFailed;
   ...
   if (abs(d0close) > 0.005  ||
       !((wz > 800  &&  abs(z0close) < 0.075) || (wz < 800  &&  abs(wz) < 0.075))) return ActionBase::kFailed;
   ...
   for (FATable<NavShower>::const_iterator shower_iter = showers.begin();
	shower_iter != showers_end;  ++shower_iter) {
	  ...
	  shower_iter->attributes().energy() > 0.020    ) num_showers++;
   }
   ...
   for (FATable<NavShower>::const_iterator shower_iter = showers.begin();
	shower_iter != showers_end;  ++shower_iter) {
      ...
      v[k_trigger] = ((triggerDecision->isElTrack() ? 100 : 0) +
		      (triggerDecision->isRadTau()  ?  10 : 0) +
		      (triggerDecision->isHadron()  ?   1 : 0)  );
      ...
      v[k_wz] = wz;
      v[k_z0close] = z0close;
      v[k_showers] = num_showers;
      v[k_first] = first;
      first = 0;

      v[k_ok] = ((shower_iter->attributes().hot()    ? 10 : 0) +
		 (shower_iter->attributes().status() ?  1 : 0)  );
      v[k_energy] = shower_iter->attributes().energy();
      v[k_theta] = shower_iter->attributes().theta();
      v[k_phi] = shower_iter->attributes().phi();
      v[k_e9oe25] = shower_iter->attributes().e9oe25();
	 ...
	 double closeness = 100000.;
	 FATable<NavTkShMatch>::const_iterator closest = matches_end;
	 for (FATable<NavTkShMatch>::const_iterator match_iter = matches.begin();
	      match_iter != matches_end;  ++match_iter) {
	    if (match_iter->distance() < closeness) {
	       closeness = match_iter->distance();
	       closest = match_iter;
	    } // end if we have a new winner
	 } // end loop looking for the closest

	 if (closest != matches_end) {
	    v[k_m_eop] = closest->eOverP();
	    v[k_m_dist] = closest->distance();
	    v[k_m_p] = closest->navTrack().pionFit()->momentum().mag()
	       * (closest->navTrack().pionHelix()->curvature() > 0 ? 1. : -1.);
	    v[k_m_d0] = closest->navTrack().pionHelix()->d0();
	    v[k_m_z0] = closest->navTrack().pionHelix()->z0();
	 } // end if we don't have a dud
      } // end has a track match

      v[k_cellid] = shower_iter->attributes().bigCellId();
...
/*
ActionBase::ActionResult
ShowerStudy::beginRun( Frame& iFrame )       // anal2 equiv.
{
...
/*
ActionBase::ActionResult
ShowerStudy::endRun( Frame& iFrame )         // anal4 equiv.
{
...
// 	 v[k_tr1p] = tr1->pionHelix()->

// 	 KTHelix movetrack1(*tr1->exitPionHelix());
// 	 double distance_moved;
// 	 if (movetrack1.moveToRadius(1.03, distance_moved, KTMoveControl::kDirectionEither) == KTMoveControl::kMoveOK) {
// 	    v[k_t1theta] = movetrack1.referencePoint().theta();
// 	    v[k_t1phi] = movetrack1.referencePoint().phi();
// 	 }

// 	 if (v[k_tracks] > 1) {
// 	    FATable<NavTrack>::const_iterator tr2 = tracks.find(props->firstTrackIdentifier());

// 	    KTHelix movetrack2(*tr2->exitPionHelix());
// 	    double distance_moved;
// 	    if (movetrack2.moveToRadius(1.03, distance_moved, KTMoveControl::kDirectionEither) == KTMoveControl::kMoveOK) {
// 	       v[k_t2theta] = movetrack2.referencePoint().theta();
// 	       v[k_t2phi] = movetrack2.referencePoint().phi();
// 	    }
// 	 } // if there are at least two tracks
//       } // if there's at least one track

DABoolean ShowerStudy::calc_intersection(double& x, double& y, double& z, double& sig2z, double& zsep,
				      KTHelix& a, KTHelix& b)
{
...
DABoolean ShowerStudy::arclength_test(double x, double y, KTHelix& a, KTHelix& b)
{
