...
// $Id: MCRawZDHitsProxy.cc,v 1.8 2003/04/11 20:38:07 bkh Exp $
//
// Revision history
//
...
#include "ZDOffCal/DriftFunction.h"
				...
				const DataKey& iKey      )
{
   ...
   report(INFO, kReport) << "There are " << responses.size() 
			 << " ZD Responses" 
   ...
   if ( numResponses != 0 )
   {
      report(INFO, kReport) << "Average efficiency of ZD Responses = "
...
//bkh   FAItem< AZDSenseWireStore > senseWireStore;
//bkh   extract( (iRecord.frame()).record(Stream::kZDAlignment), senseWireStore);

   // Extract the drift function
//bkh   FAItem<DriftFunction> calibration;
//bkh   extract( (iRecord.frame()).record(Stream::kBeginRun), calibration);
...
//bkh     ZDDatum::DigiType adc = calibration->charge2adc(cellId, totalCharge );
     ZDDatum::DigiType adc = 3333 ;//bkh
     ...
     // (from the cell id)
//bkh     AZDSenseWireStore::LayerWire layerWire 
//bkh       = senseWireStore->layerWire(cellId);
//bkh     double layerRadius = senseWireStore->radiusNominal(layerWire.first);
     
     for ( unsigned int j = 0; j < times.size(); ++j )
     {
       
       // Convert the time to tdc
//bkh	ZDDatum::DigiType tdc = calibration->time2tdc(cellId, times[j], layerRadius);
	ZDDatum::DigiType tdc = 2999 ;
