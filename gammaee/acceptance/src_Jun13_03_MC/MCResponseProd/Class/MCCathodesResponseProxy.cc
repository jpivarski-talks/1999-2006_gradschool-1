...
// $Id: MCCathodesResponseProxy.cc,v 1.16 2003/03/07 21:55:07 cdj Exp $
//
// Revision history
//
// $Log: MCCathodesResponseProxy.cc,v $
// Revision 1.16  2003/03/07 21:55:07  cdj
// converted huge areas to local static to avoid stack overflows
//
// Revision 1.15  2003/03/05 18:31:39  cdj
// protect against divide by zero and fixed code indentation
//
...
static const char* const kIdString  = "$Id: MCCathodesResponseProxy.cc,v 1.16 2003/03/07 21:55:07 cdj Exp $";
...
//=====================================================================
   ...
   //These are static since allocating them on the stack can cause
   // stack overflows (since this is more than 6MB of memory!)
   //
   // Having this as static means this routine is not re-entrant
   //  but Proxies really should not be calling themselves but 
   //  it also means that there could be problems running multiple
   //  instances of this Proxy (such as if we tried to Merge different
   //  MC events)
   //
   // A better design would be to make the data in these arrays into
   //  a class and then have a std::vector of those objects and have
   //  the vector be a member data for efficiency reasons
   static int   drIDR[numAnodeCollections];
   static float xR[numAnodeCollections][numQDeps];
   static float yR[numAnodeCollections][numQDeps]; 
   static float zR[numAnodeCollections][numQDeps]; 
   static float qR[numAnodeCollections][numQDeps]; 
   static float dipR[numAnodeCollections][numQDeps];  
   static int nAnodes[numAnodeCollections];
   static int pidR[numAnodeCollections];
   static int layR[numAnodeCollections];
   static int wireR[numAnodeCollections];
   int iResp = 0; 
   int iLoop = 0;
   float minRadius=10000.;
   static float qTotalR[numAnodeCollections];   
   static float mR[numAnodeCollections];
   static float mZ[numAnodeCollections];
   static float mDipR[numAnodeCollections];
   static float mPhiR[numAnodeCollections];
   int  exitPadId = -1;
   int  entrPadId = -1;
   float exitPadZ = 0.;
   float entrPadZ = 0.;
...
// get important Zs of Cell
   static float cellZR[numAnodeCollections];
   static float exitRadius[numAnodeCollections];
   static float entrRadius[numAnodeCollections];
   static float exitZ[numAnodeCollections];
   static float entrZ[numAnodeCollections];
   static float exitZDif[numAnodeCollections];
   static float entrZDif[numAnodeCollections];
   float wireRadius = 0.;
...
// assume half way between 46 and 47 sense wires
	...
	float r46=xyzL46.perp();
	float r47=xyzL47.perp();
...
//      float Scale = 0.15; 
	...
	static float padQ[ADRCathodeStore::kNumberOfPadsTotal+1];
	static float padZ[ADRCathodeStore::kNumberOfPadsTotal+1];
...
// of pads in Z.
	  ...
	  report(DEBUG, kReport) 
	     << " IdOfLastPadInPanel " << IdOfLastPadInPanel << endl;
	  report(DEBUG, kReport) 
	     << " IdOfFirstPadInPanel " << IdOfFirstPadInPanel << endl;
...
// "a" should be the dist of the wire from the cathode in units of pads
	...
	for(i=1; i<ADRCathodeStore::kNumberOfPadsTotal+1; i++)
	{
	  if(padQ[i]>0.){
	    ...
	    if(padQ[i]<0.){ 
	      report(DEBUG, kReport) << "negative charge on cathode" << endl;
	    }
	  }
	}
      }
