...
// $Id: G3iMakeTrackingMedia.cc,v 1.8 2002/11/27 21:06:50 bkh Exp $
//
// Revision history
//
...
//
// Revision 1.7  2002/10/21 21:27:08  bkh
// add char string length in fortran call
//
...
//      PARAMETER ( SVSTEP = SVUSTP / SVNSTP )
   ...
   // DR WireHoles
   const FInteger  kDRWireHolesMatIndex  
      ( ( *(matMap.find( kDRWireHolesMatID ) ) ).second ) ;
   const FInteger  kDRWireHolesSensitive ( 1          ) ;
   const FReal     kDRWireHolesBFieldMax ( solBField  ) ;
   const FReal     kDRWireHolesAngleMax  ( stdAngle   ) ;
   const FReal     kDRWireHolesTrkPrec   ( stdTrkPrec ) ;
   const FReal     kDRWireHolesMScatMax  ( stdMScat   ) ;
   const FReal     kDRWireHolesELossMax  ( stdELossMx ) ;
   const FReal     kDRWireHolesELossMin  ( stdELossMn ) ;
   const DABoolean kDRWireHolesLoCuts    ( true       ) ;
   const DABoolean kDRWireHolesNoDelta   ( false      ) ;
   const DABoolean kDRWireHolesUserParm  ( false      ) ;
