...
// $Id: MCRawMUHitsProxy.cc,v 1.7 2001/05/03 20:58:48 savinov Exp $
//
// Revision history
//
...
//
MCRawMUHitsProxy::MCRawMUHitsProxy( const Parameter<DABoolean>& m_makeMCRawMUHits) :
   m_RawMUHits( 0 ) ,
   m_words    ( *new STL_VECTOR( UInt32 ) ) 
{
   iMakeMCRawMUHits = m_makeMCRawMUHits.value();

   report( DEBUG, kReport ) << "iMakeMCRawMUHits: " << iMakeMCRawMUHits << endl;
