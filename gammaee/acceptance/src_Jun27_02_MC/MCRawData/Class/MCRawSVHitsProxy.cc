...
// $Id: MCRawSVHitsProxy.cc,v 1.15 2001/12/20 17:11:20 bkh Exp $
//
// Revision history
//
...
static const char* const kIdString  = "$Id: MCRawSVHitsProxy.cc,v 1.15 2001/12/20 17:11:20 bkh Exp $";
     ...
     
{
   ...
   report( INFO, kReport ) << "In the constructor of  MCRawSVHitsProxy..."<<endl;

   iMakeMCRawSVHits = getMCRawSVHits.value();
   m_useFakeMerge   = useFakeMerge.value();
   m_CMNScaleFactor = CMNoiseValue.value();
				...
				const DataKey& iKey )
{
   ...
   if (iMakeMCRawSVHits != 0) 
   {
	 ...
	 else
	 {
      ...
      float commonNoise =  m_CMNScaleFactor*ran.gauss(0, 1.0);
	 ...
	 if(  CopySiSparsifierAccessories::getEventCount() % 
	      CopySiSparsifierAccessories::SVSparNKeep != 0 )
	 {
	 ...
	 {
	    m_words.push_back( *(pCrate1+crate1Counter) ) ;
	 }
	 
	 for( UInt32 crate2Counter = 0; crate2Counter < wordsWrittenCrate2; crate2Counter++)
	 {
	    m_words.push_back( *(pCrate2+crate2Counter) ) ;
	 }
      }
      else
      {
