...
// $Id: testPack.cc,v 1.8 2002/03/17 16:06:33 cdj Exp $
//
// Revision history
//
// $Log: testPack.cc,v $
...
class PrecisionCompare : public Compare<T>
{
   public:
	 ...
	 if( iTrue > m_max ) {
	    ...
	    return (iStored >= m_max && iStored <= iTrue);
	 }
	 if( iTrue < m_min ) {
	    cout << "*** true < min -- out of range" << endl;
	    return (iStored <= m_min && iStored >= iTrue);
	 }
...
class FractionCompare : public Compare<T>
{
   public:
	 ...
	 if( iTrue < -1.0*m_max ) {
	    cout << "*** true < -max -- out of range" << endl;
	    return (iStored <= -1.0*m_max && iStored >= iTrue);
	 }
...
check_stored_value( const T* iDummy,
   ...
   if( iPackStuff.m_type == 
	 ...
	 static_cast<SMPackerTemplate<T>* const>(iPackStuff.m_packer)->unpack(
      ...
      if( reinterpret_cast<Compare<T>* const >(iPackStuff.m_comparer)->areEqual( 
	 value, temp ) ) {
