...
tcl_proc cleog_file args {
   ...
   if { 0 == [string compare $rpExtension [string range $fileName end-[expr [string length $rpExtension] -1] end] ] } {
       ...
       if [catch {
	   set argsArray(-post) [linsert $argsArray(-post) 0 "param TrackletBunchFinder ForceTimeOffset true; param TrackletBunchFinder TheForcedTimeOffset 0.0;prod desel CesrBeamEnergyProd;source_format desel BeamSpotFileSourceFormat;"]
       } ] {
	   set argsArray(-post) "param TrackletBunchFinder ForceTimeOffset true; param TrackletBunchFinder TheForcedTimeOffset 0.0;prod desel CesrBeamEnergyProd;source_format desel BeamSpotFileSourceFormat;"
       }
...
tcl_proc cleog_out_file {filename } {
    global env
    run_file scripts_$env(CMLIB)/controlMCWriteout_cleog.tcl
