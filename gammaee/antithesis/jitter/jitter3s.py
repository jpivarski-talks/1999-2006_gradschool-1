...
#   cs = u3func(3.63925, 4.479665, 0.404231, 0.0263, 0.01823, 0., 0.0239, 0.27, 0., 0.0792, 13.6819, 5.7247, average_energy*2000.) + random.gauss(0., cs_err)
  cs = u3func(3.63925, 4.479665, 0.404231, 0.0263, 0.01823, 0., 0.0239, 0.27, 0., 0.0792, 13.6819, 5.7247, average_energy*2000.) + random.gauss(0., cs_err)
...
for jitter in (0., 0.05, 0.07):
  ...
  for zoinkies in range(trials):
    u3data["nov28"] = setup_data(u3runs["nov28"], jitter)
    u3data["dec05"] = setup_data(u3runs["dec05"], jitter)
    u3data["dec12"] = setup_data(u3runs["dec12"], jitter)
    u3data["dec19"] = setup_data(u3runs["dec19"], jitter)
    u3data["dec26"] = setup_data(u3runs["dec26"], jitter)
    u3data["jan02"] = setup_data(u3runs["jan02"], jitter)
    u3data["jan09"] = setup_data(u3runs["jan09"], jitter)

    u3fitter = Minuit(u3fit)
    u3fitter.values = [3.63925, 4.479665, 0.404231, 0., 0., 0., 0., 0., 0., 0., 0.0263, 0.01823, 0., 0.0239, 0.27, 0., 0.0792, 13.6819, 5.7247]
    ...
    u3fitter.migrad()
    if u3fitter.valid:
      print "area =", u3fitter.values[0], "with chi^2", u3fitter.fval
      areas.append(u3fitter.values[0])
      chi2s.append(u3fitter.fval)
...
#   chi2s = []
#   for zoinkies in range(trials):
#     u3data["nov28"] = setup_data_cs(u3runs["nov28"], jitter)
#     u3data["dec05"] = setup_data_cs(u3runs["dec05"], jitter)
#     u3data["dec12"] = setup_data_cs(u3runs["dec12"], jitter)
#     u3data["dec19"] = setup_data_cs(u3runs["dec19"], jitter)
#     u3data["dec26"] = setup_data_cs(u3runs["dec26"], jitter)
#     u3data["jan02"] = setup_data_cs(u3runs["jan02"], jitter)
#     u3data["jan09"] = setup_data_cs(u3runs["jan09"], jitter)
