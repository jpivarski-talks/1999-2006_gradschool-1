...
#  cs = u2func(5.7247, 4.1758, 0.40496, 0.043, 0.01792, 0., 0.017, 0.37, 0., 0.0792, 13.6819, average_energy*2000.) + random.gauss(0., cs_err)
  cs = u2func(5.7247, 4.1758, 0.40496, 0.043, 0.01792, 0., 0.017, 0.37, 0., 0.0792, 13.6819, average_energy*2000.) + random.gauss(0., cs_err)
...
for jitter in (0., 0.05, 0.07):
  ...
  for zoinkies in range(trials):
    u2data["may29"] = setup_data(u2runs["may29"], jitter)
    u2data["jun11"] = setup_data(u2runs["jun11"], jitter)
    u2data["jun12"] = setup_data(u2runs["jun12"], jitter)
    u2data["jul10"] = setup_data(u2runs["jul10"], jitter)
    u2data["jul24"] = setup_data(u2runs["jul24"], jitter)
    u2data["aug07"] = setup_data(u2runs["aug07"], jitter)

    u2fitter = Minuit(u2fit)
    u2fitter.values = [5.7247, 4.1758, 0.40496, 0., 0., 0., 0., 0., 0., 0.043, 0.01792, 0., 0.017, 0.37, 0., 0.0792, 13.6819]
    u2fitter.fix(range(u2fitter.npar))
    u2fitter.release(["area", "rmsbeam", "back"])
    u2fitter.release(["may29", "jun11", "jun12", "jul10", "jul24", "aug07"])
    u2fitter.migrad()
    if u2fitter.valid:
      print "area =", u2fitter.values[0], "with chi^2", u2fitter.fval
      areas.append(u2fitter.values[0])
      chi2s.append(u2fitter.fval)
...
#   chi2s = []
#   for zoinkies in range(trials):
#     u2data["may29"] = setup_data_cs(u2runs["may29"], jitter)
#     u2data["jun11"] = setup_data_cs(u2runs["jun11"], jitter)
#     u2data["jun12"] = setup_data_cs(u2runs["jun12"], jitter)
#     u2data["jul10"] = setup_data_cs(u2runs["jul10"], jitter)
#     u2data["jul24"] = setup_data_cs(u2runs["jul24"], jitter)
#     u2data["aug07"] = setup_data_cs(u2runs["aug07"], jitter)
