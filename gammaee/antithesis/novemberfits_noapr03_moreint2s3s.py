...
def doitall(lumisource, bhabha_interference, theyint=None, thephi=None):
  def run_date(r):
...
# #  u1fitter.values[u1fitter.findpar("yint")] = 0.0560
# #  u1fitter.values[u1fitter.findpar("yint")] = 0.042

#   if theyint != None:
#     u1fitter.values[u1fitter.findpar("yint")] = theyint
# #  u1fitter.values[u1fitter.findpar("fullgam")] = (53.0+1.5)/1000.

#   if thephi != None:
#     u1fitter.values[u1fitter.findpar("phi")] = thephi
...
#   print "remember: chi^2 first, then area"
#   if not u1fitter.valid: print "************************", "FAILED!"
#   else: print "************************", u1fitter.fval, u1fitter.values[0]
#   return None
...
#                        float(rjan_weight + rfeb_weight + rapr2_weight)
  ...
  u2fitter.values = [134.14, 4.187, 9.318, -1.31, -1.21, -1.14, -0.80, -0.81, -0.66, 0.043, 0.01792, 0.0, 0.0203, 0.239713, 0.0, 0.0792, ggfits[1].values[0]]
...
#  u2fitter.values[u2fitter.findpar("yint")] = 0.0772
#  u2fitter.values[u2fitter.findpar("yint")] = 0.057
  if theyint != None:
    u2fitter.values[u2fitter.findpar("yint")] = theyint
  if thephi != None:
    u2fitter.values[u2fitter.findpar("phi")] = thephi

#  u2fitter.values[u2fitter.findpar("fullgam")] = (30.5)/1000.
...
#  print zip(u2fitter.names, u2fitter.minos_errors)
  ...
  u3fitter.values = [89.5, 4.68, 9.315, -3.36, -3.96, -2.85, -2.74, -2.74, -2.90, -2.99, 0.0263, 0.01823, 0.0, 0.0239, 0.27, 0.0, 0.0792, ggfits[1].values[0], ggfits[2].values[0], 4.66, 4.69, 4.68, 4.74, 4.80, 4.52, 4.68]
...
#  u3fitter.values[u3fitter.findpar("yint")] = 0.0652
#  u3fitter.values[u3fitter.findpar("yint")] = 0.048
  if theyint != None:
    u3fitter.values[u3fitter.findpar("yint")] = theyint
  if thephi != None:
    u3fitter.values[u3fitter.findpar("phi")] = thephi

#  u3fitter.values[u3fitter.findpar("fullgam")] = (18.6)/1000.
...
# amp_factors = list(Numeric.arange(3., 0., -0.2))
# amp_factors.reverse()
# for amp_factor in amp_factors:
#   for phig in Numeric.arange(0., 360., 5.):
#     yintg = amp_factor * 0.0174
#     effective_yint = sqrt(0.0174**2 + yintg**2 + 2.*0.0174*yintg*cos(phig*pi/180.))
#     effective_phi = 180./pi*atan2(yintg*sin(phig*pi/180.), 0.0174 + yintg*cos(phig*pi/180.))
#     print "************************", amp_factor, phig, "... GO!"
#     doitall(3, 1., effective_yint, effective_phi)
...
effective_yint = sqrt(0.0174**2 + yintg**2 + 2.*0.0174*yintg*cos(phig*pi/180.))
effective_phi = 180./pi*atan2(yintg*sin(phig*pi/180.), 0.0174 + yintg*cos(phig*pi/180.))
print "***************************** First the experimental fit."
doitall(3, 1., effective_yint, effective_phi)
print "***************************** Now a control fit."
doitall(3, 1., 0.0174, 0.)

# amp_factors = list(Numeric.arange(3., 0., -0.2))
# amp_factors.reverse()
# for amp_factor in amp_factors:
#   for phig in Numeric.arange(0., 360., 22.5):
#     yintg = amp_factor * 0.0174
#     effective_yint = sqrt(0.0174**2 + yintg**2 + 2.*0.0174*yintg*cos(phig))
#     effective_phi = atan2(yintg*sin(phig), 0.0174 + yintg*cos(phig))

#    print "+++++++++++++++++", amp_factor, yintg, phig, "... GO!"
#    doitall(3, 1., effective_yint, effective_phi)
