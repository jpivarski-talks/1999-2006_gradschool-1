...
def setup_data(runs, fudge=1., limit=None):
  runs.sort()
  beginning = run_date(runs[0])

  tmpdata = []
  for r in runs:
    if limit == None or run_date(r) < beginning + limit:
...
#     if 1. * nobeam.hadroncool * therun.cosmic / nobeam.cosmic / therun.gamgam / 7.55 > 0.01:
#       print "run", r, "has", 1. * nobeam.hadroncool * therun.cosmic / nobeam.cosmic / therun.gamgam / 7.55 * 100., "percent cosmic rays at beam energy", runsummary[r].energy
...
#         continue
  ...
  if g == 0: return average_energy, 0., 0.
...
# for hours in (24, 32, 40, 48, 56, 64, 72, 80, 88, 96):
#   u1data["jan16"] = setup_data(u1runs["jan16"], limit=hours*60.*60.)
#   u1data["jan30"] = setup_data(u1runs["jan30"], limit=hours*60.*60.)
#   u1data["feb06"] = setup_data(u1runs["feb06"], limit=hours*60.*60.)
#   u1data["feb13"] = setup_data(u1runs["feb13"], limit=hours*60.*60.)
#   u1data["feb20"] = setup_data(u1runs["feb20"], limit=hours*60.*60.)
#   u1data["feb27"] = setup_data(u1runs["feb27"], limit=hours*60.*60.)
#   u1data["mar06"] = setup_data(u1runs["mar06"], limit=hours*60.*60.)
#   u1data["mar13"] = setup_data(u1runs["mar13"], limit=hours*60.*60.)
#   u1data["apr03"] = setup_data(u1runs["apr03"], limit=hours*60.*60.)
#   u1data["apr08"] = setup_data(u1runs["apr08"], limit=hours*60.*60.)
#   u1data["apr09"] = setup_data(u1runs["apr09"], limit=hours*60.*60.)
#   u1data["apr10"] = setup_data(u1runs["apr10"], limit=hours*60.*60.)
...
#   print "u1: "+u1fitter.whystring+" "+u1fitter.minos_whystring+" Hours =", hours, ", area uncertainty =", u1fitter.minos_errors[0], " MeV nbish"

# for hours in (24, 32, 40, 48, 56, 64, 72, 80, 88, 96):
#   u2data["may29"] = setup_data(u2runs["may29"], limit=hours*60.*60.)
#   u2data["jun11"] = setup_data(u2runs["jun11"], limit=hours*60.*60.)
#   u2data["jun12"] = setup_data(u2runs["jun12"], limit=hours*60.*60.)
#   u2data["jul10"] = setup_data(u2runs["jul10"], limit=hours*60.*60.)
#   u2data["jul24"] = setup_data(u2runs["jul24"], limit=hours*60.*60.)
#   u2data["aug07"] = setup_data(u2runs["aug07"], limit=hours*60.*60.)
...
#   print "u2: "+u2fitter.whystring+" "+u2fitter.minos_whystring+" Hours =", hours, ", area uncertainty =", u2fitter.minos_errors[0], " MeV nbish"

# for hours in (24, 32, 40, 48, 56, 64, 72, 80, 88, 96):
#   u3data["nov28"] = setup_data(u3runs["nov28"], limit=hours*60.*60.)
#   u3data["dec05"] = setup_data(u3runs["dec05"], limit=hours*60.*60.)
#   u3data["dec12"] = setup_data(u3runs["dec12"], limit=hours*60.*60.)
#   u3data["dec19"] = setup_data(u3runs["dec19"], limit=hours*60.*60.)
#   u3data["dec26"] = setup_data(u3runs["dec26"], limit=hours*60.*60.)
#   u3data["jan02"] = setup_data(u3runs["jan02"], limit=hours*60.*60.)
#   u3data["jan09"] = setup_data(u3runs["jan09"], limit=hours*60.*60.)
...
#   print "u3: "+u3fitter.whystring+" "+u3fitter.minos_whystring+" Hours =", hours, ", area uncertainty =", u3fitter.minos_errors[0], " MeV nbish"

# u1data["jan16"] = setup_data(u1runs["jan16"], limit=None)
# u1data["jan30"] = setup_data(u1runs["jan30"], limit=None)
# u1data["feb06"] = setup_data(u1runs["feb06"], limit=None)
# u1data["feb13"] = setup_data(u1runs["feb13"], limit=None)
# u1data["feb20"] = setup_data(u1runs["feb20"], limit=None)
# u1data["feb27"] = setup_data(u1runs["feb27"], limit=None)
# u1data["mar06"] = setup_data(u1runs["mar06"], limit=None)
# u1data["mar13"] = setup_data(u1runs["mar13"], limit=None)
# u1data["apr03"] = setup_data(u1runs["apr03"], limit=None)
# u1data["apr08"] = setup_data(u1runs["apr08"], limit=None)
# u1data["apr09"] = setup_data(u1runs["apr09"], limit=None)
# u1data["apr10"] = setup_data(u1runs["apr10"], limit=None)
...
# u2data["may29"] = setup_data(u2runs["may29"], limit=48.*60.*60.)
# u2data["jun11"] = setup_data(u2runs["jun11"], limit=48.*60.*60.)
# u2data["jun12"] = setup_data(u2runs["jun12"], limit=48.*60.*60.)
# u2data["jul10"] = setup_data(u2runs["jul10"], limit=48.*60.*60.)
# u2data["jul24"] = setup_data(u2runs["jul24"], limit=48.*60.*60.)
# u2data["aug07"] = setup_data(u2runs["aug07"], limit=48.*60.*60.)

# u2fitter = Minuit(u2fit)
# u2fitter.values = [5.59, 4.17, 0.405, -1.12, -1.05, -1.41, -0.75, -0.65, -0.39, 0.043, 0.01792, 0., 0.017, 0.37, 0., 0.0792, 13.68] # u1fitter.values[0]]
# u2fitter.fix(range(u2fitter.npar))
# u2fitter.release(["area", "rmsbeam", "back"])
# u2fitter.release(["may29", "jun11", "jun12", "jul10", "jul24", "aug07"])
# u2fitter.migrad()
# # if u2fitter.valid: u2fitter.minos("area")
# # print "u2: "+u2fitter.whystring+" "+u2fitter.minos_whystring+" Hours =", hours, ", area uncertainty =", u2fitter.minos_errors[0], " MeV nbish"
# print "u2 area: ", u2fitter.values[0]

u3data["nov28"] = setup_data(u3runs["nov28"], limit=48.*60.*60.)
u3data["dec05"] = setup_data(u3runs["dec05"], limit=48.*60.*60.)
u3data["dec12"] = setup_data(u3runs["dec12"], limit=48.*60.*60.)
u3data["dec19"] = setup_data(u3runs["dec19"], limit=48.*60.*60.)
u3data["dec26"] = setup_data(u3runs["dec26"], limit=48.*60.*60.)
u3data["jan02"] = setup_data(u3runs["jan02"], limit=48.*60.*60.)
u3data["jan09"] = setup_data(u3runs["jan09"], limit=48.*60.*60.)

u3fitter = Minuit(u3fit)
u3fitter.values = [3.58, 4.48, 0.404, -2.12, -3.99, -3.18, -2.46, -2.72, -2.55, -2.64, 0.0263, 0.01823, 0., 0.0239, 0.27, 0., 0.0792, 13.68, 5.737] # u1fitter.values[0], u2fitter.values[0]]
u3fitter.fix(range(u3fitter.npar))
u3fitter.release(["area", "rmsbeam", "back"])
u3fitter.release(["nov28", "dec05", "dec12", "dec19", "dec26", "jan02", "jan09"])
u3fitter.migrad()
# if u3fitter.valid: u3fitter.minos("area")
# print "u3: "+u3fitter.whystring+" "+u3fitter.minos_whystring+" Hours =", hours, ", area uncertainty =", u3fitter.minos_errors[0], " MeV nbish"
print "u3 area: ", u3fitter.values[0]
