...
// $Id: KalmanTrackFitter.cc,v 1.54 2002/02/22 03:32:43 wsun Exp $
//
// Revision history
//
...
KalmanTrackFitter::makeConditionsAndOperations() 
{
   ...
   if( m_applyMultScat || m_applyDedx )
   {
      ...
      HICorrectForBFieldOperation* bfield0 = new HICorrectForBFieldOperation(false);
      m_fitOperation->addChild( bfield0 );
      ...
      //Adding this for Bfield corrections
      HICorrectForBFieldOperation* bfield1 = new HICorrectForBFieldOperation(false);
      m_fitSiOperation->addChild( bfield1 );
      ...
      //Adding this for Bfield corrections
      HICorrectForBFieldOperation* bfield2 = new HICorrectForBFieldOperation(false);
      m_noFitOperation->addChild( bfield2 );
      ...
      //Adding this for Bfield corrections      
      HICorrectForBFieldOperation* bfield3 = new HICorrectForBFieldOperation(false);
      m_noFitSiOperation->addChild( bfield3 );
   ...
   if( m_applyDedx && m_maxFracDP < 1. )
   {
      ...
      //Adding this for Bfield corrections 
      HICorrectForBFieldOperation* bfield4 = new HICorrectForBFieldOperation(false);
      m_fitOperation->addChild( bfield4 );
...
//   intersector.setAttemptCurling( false ) ;
   ...
   for( int iHitDel = 0 ; iHitDel <= imax ; ++iHitDel )
   {
	 ...
	 m_inwardFit->reset() ;
      ...
      // m_inwardFit (which is actually the inward fit if m_inputTrackInfo
      // is kAddHitsToInputTrackInside).  So, later on, m_inwardFit and
	 ...
	 // minimizes the z-distance from the IP.  If m_inputTrackInfo is
	 // kAddHitsToInputTrackInside,the inward fit is actually m_inwardFit.

	 if( !m_inwardFitsAtLastHit )
	 {
	    if( m_inputTrackInfo == kAddHitsToInputTrackInside )
	    {
	       m_inwardFit->moveToReferencePoint(
		  HepPoint3D( 0., 0., 0. ),
		  KTMoveControl::kDirectionBackward ) ;
	    }
	    else
