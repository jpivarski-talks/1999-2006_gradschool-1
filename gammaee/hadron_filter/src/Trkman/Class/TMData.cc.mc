...
//
TMData::TMData( NavTrackTable& theTracks, FAItem< MCDecayTree >& decayTree )
{

   //Set the event vertex
   MCDecayTree::const_vIterator eVertex = (*decayTree).findVertexWithParent( "VPHO", true, kiAnyInteraction );

   if( eVertex != (*decayTree).vEnd() )
      m_eventVertex = (*eVertex).position();
   else report( WARNING, kFacilityString ) << "Could not find the event vertex!!" << endl;
...
//    m_dbMaxCut   = 0.0015;
   ...
   for( nTrack = m_theTracks.begin(); nTrack != m_theTracks.end(); nTrack++ )
   {
      ...
      //m_hits[i]   = (double) nTrack->seedQuality()->numberHits();
	 ...
	 m_densityOfHits[i] = -1.0;	 
      ...
      status = aHelix->moveToReferencePoint( m_eventVertex, dist_moved );
