...
// $Id: HIMSDedxOperation.cc,v 1.7 2002/02/21 07:17:48 wsun Exp $
//
// Revision history
//
...
static const char* const kIdString  = "$Id: HIMSDedxOperation.cc,v 1.7 2002/02/21 07:17:48 wsun Exp $";
...
//    }
   ...
   if( m_applyDedx )
   {
	 ...
	 sumOfWeights += weight ;
      ...
      for( iter = aMaterial->elements().begin() ;
	 ...
	 // Density in g/cm^3
//	 double density = aMaterial->density()/1000. * weight/sumOfWeights ;
	 ...
	 {
	    I = m_ionizationPotentials[ int( element.m_atomicNumber ) - 1 ] ;
	    logI =
	 ...
	 GeV dEdx = energyLossPerPathLength( element.m_atomicNumber,
					     rhoZOverA,
					     I,
					     logI,
					     C,
	 ...
	 {
	    kinE = energy - mass ;
	    dEdx = energyLossPerPathLength( element.m_atomicNumber,
					    rhoZOverA,
					    I,
					    logI,
					    C,
