...
// $Id: TMClassification_Vees.cc,v 1.4 2002/11/13 15:56:52 nea9 Exp $
//
// Revision history
//
// $Log: TMClassification_Vees.cc,v $
// Revision 1.4  2002/11/13 15:56:52  nea9
// Bug fix, changed names of variables called and and xor,
// to pairAND, pairXOR, since and and xor are now C++ names.
//
...
DABoolean TMClassification::calculateTrackHitDensities( int i, int j )
{
   ...
   for( int nn=1; nn<=lastL; nn++ )
   {
      ...
      if(  iHit && jHit ) pairAND++;
      else if( iHit || jHit ) pairXOR++;
   }
   ...
   //Tight
   if( !( pairAND<6 && pairXOR<15 ) ) return false; 
