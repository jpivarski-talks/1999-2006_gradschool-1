...
// $Id: BeamGasFilterProc.cc,v 1.7 2003/03/10 17:54:26 mccann Exp $
//
// Revision history
//
...
static const char* const kIdString  = "$Id: BeamGasFilterProc.cc,v 1.7 2003/03/10 17:54:26 mccann Exp $";
...
void BeamGasFilterProc::printout(Severity severity, DABoolean runOnly)
{
   ...
   if ( m_closestIntersectionCut.value() == closestIntersectionDefault  &&
	...
	m_weightedZCleverness.value()    == weightedZClevernessDefault     ) {
      ...
      double contaminationStat( sqrt(shoulder) * m_centerFactor.value() );
      ...
      double cosmic_contaminationStat( sqrt(cosmic_shoulder) * m_cosmicCenterFactor.value() );
	 ...
	 << "Passed " << passed << " out of " << total << " events, or " << fraction*100. << " %." << endl;
      if ( runOnly ) report(severity, kFacilityString) << "RUN_CHECK for " << run << " : Last event was " << m_event << endl;
