...
// $Id: TrackFinder.cc,v 1.15 2002/09/27 21:02:46 bkh Exp $
//
// Revision history
//
// $Log: TrackFinder.cc,v $
// Revision 1.15  2002/09/27 21:02:46  bkh
// Add ZD capability
//
...
STL_VECTOR(const CalibratedZDHit*)* TrackFinder::filterZDHits(
   const Record& theRecord, const char* const zdHitUsageTag ) const
{
   ...
   {
      // Extract the hits
      FATable<CalibratedZDHit> allOfTheZDHits;
      try
      {
	 // Use a usage tag if we got it
	 if ( 0 != zdHitUsageTag )
	 {
	    extract( theRecord, allOfTheZDHits, zdHitUsageTag );
	 }
	 else
	 {
	    extract( theRecord, allOfTheZDHits );
	 }
      }
	 ...
	 else
	 {
      ...
      // Make the vector of hits on which to run tracking 
      theZDHits = new STL_VECTOR(const CalibratedZDHit*);

      // Did we actually get the hits (did the extract fail and the 
      // exception caught)
      if ( allOfTheZDHits.valid() )
      {
	 // Loop over hits and put them into vector
	 for ( FATable<CalibratedZDHit>::const_iterator iter = 
		  allOfTheZDHits.begin();
	       iter != allOfTheZDHits.end(); ++iter )
	 {
	    theZDHits->push_back(&(*(iter))); 
	 }
      }
   }
   // done
...
{
   STL_VECTOR(const CalibratedSVRphiHit*)* theSVRphiHits ( 0 ) ;
   ...
   if( iConfig->useSInotZD() )
   {
	 ...
	 else
	 {
      ...
      // Make the vector of hits on which to run tracking 
      theSVRphiHits = new STL_VECTOR(const CalibratedSVRphiHit*);
...
STL_VECTOR(const CalibratedSVZHit*)* TrackFinder::filterSVZHits(
   const Record& theRecord ) const
{
   STL_VECTOR(const CalibratedSVZHit*)* theSVZHits ( 0 ) ;
   ...
   if( iConfig->useSInotZD() )
   {
	 ...
	 else
	 {
      ...
      // Make the vector of hits on which to run tracking 
      theSVZHits = new STL_VECTOR(const CalibratedSVZHit*);
