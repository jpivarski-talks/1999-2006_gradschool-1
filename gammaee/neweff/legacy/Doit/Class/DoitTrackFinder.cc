...
// Revision 1.68  2003/02/06 19:07:05  cdj
// removed code that was put in during last tag
//
// Revision 1.66  2002/09/27 21:01:19  bkh
// Add ZD capability and compatibility with new TrackFinder interface
//
// Revision 1.65  2002/09/05 19:41:42  dpp
//      -> remove calls to TFHINI,TFGMI0
//
// Revision 1.64  2001/12/06 23:22:30  bkh
// Remove unnecessary header file for raw hits and use better fcn for raw time
//
...
DoitTrackFinder::DoitTrackFinder() :
   ...
   m_doitZDLattice(0),  // Lattice for ZD anode wire hits
   m_doitSVRLattice(0),  // Lattice for SVR hits
   m_doitSVZLattice(0),  // Lattice for SVZ hits
...
void DoitTrackFinder::beginRun(Frame& theFrame) const
{
   ...
   extract( theFrame.record( Stream::kBeginRun ) , iConfig ) ;
   ...
   // Extract ZD geometry
//FIX!!   FAItem< ADZDSenseWireStore > zenseWireStore;
   if ( m_useZD               &&
	iConfig->useZDnotSI()    )
   {
/*
//FIX!!      extract( theFrame.record(Stream::kDRAlignment), zenseWireStore);

   if ( ! zenseWireStore.valid() )
   {
      report(EMERGENCY, kFacilityString) << "Invalid AZDSenseWireStore" 
					 << endl;
      assert(zenseWireStore.valid());
      ::exit(1);
   }
   */
   }
   ...
   // Fill zdgeometry common block
   if ( m_useZD               &&
	iConfig->useZDnotSI() )
   {
//FIX!!      fillFortranZDGeometry(*zdStore);
   }
...
DoitTrackFinder::findTracks(
   ...
   int diagLevel,
   STL_VECTOR(const CalibratedZDHit*)* theZDHits
   ) const
{

   FAItem< DetectorConfiguration >                          iConfig   ;
   extract( theRecord.frame().record( Stream::kBeginRun ) , iConfig ) ;
   ...
   if ( m_useZD               &&
	iConfig->useZDnotSI()    ) {
      if ( diagLevel > 0 )
	 report(DEBUG, kFacilityString) 
	    << "FILLING FORTRAN COMMONS with ZD Hits !!!!!!!!!!!" << endl;
      fillFortranCommons( theZDHits, theRecord.frame() );
   }
   ...
   const Frame& theFrame ) const
{
   ...
   if ( theDRHits != 0 )
   {
	 ...
	 assert(idx == l+1);
       ...
       {
	  const_cast<DoitTrackFinder*>(this)->m_hitIdentifiers.
	     push_back(hitId);
       }
       else
       {
	  const_cast<DoitTrackFinder*>(this)->m_hitIdentifiers[l] = hitId;
       }
...
//////////////////////////////////////////////////////////////////////
// Fill Common blocks for DOIT and SF with ZD Hits information
void DoitTrackFinder::fillFortranCommons( 
   STL_VECTOR(const CalibratedZDHit*)* theZDHits,
   const Frame& theFrame ) const
{
//FIX!!
   ...
   // Get geometry information for the ZD
//   FAItem< AZDSenseWireStore > senseWireStore;
//   extract(theFrame.record(Stream::kZDAlignment), senseWireStore);

   // Are there any DR hits?
   if ( theZDHits != 0 )
   {
     for ( unsigned int l = 0; l < theZDHits->size(); ++l )
     {

       const CalibratedDRHit* aZDHit = (*theZDHits)[l];

       int layer      = aZDHit->layer();
       int wire       = aZDHit->wireInLyr();
       int   tdc      = aZDHit->rawTime() ;
       float time     = aZDHit->time();
       float charge   = aZDHit->charge();
       float distance = aZDHit->distance();
       int   qual     = aZDHit->qualHit();
       int   ntdc     = aZDHit->nTDC();
       float earlyGap = aZDHit->earlyGap();
       float lateGap  = aZDHit->lateGap();
...
//       float phi = theLayer.phi0() + wire*theLayer.cellPhiWidth();
	 ...
	 phi -= 2.0*M_PI;
       ...
       float xpos ;//FIX!! = theLayer.radius() * cos(phi);
       float ypos ;//FIX!! = theLayer.radius() * sin(phi);

       
       // Determine the "logical address" of this hit (this the cellid)
       int cdla ;//FIX!!= senseWireStore->cellID(layer, wire);

       int idx ( 0 ) ;
       // Fill this hit in the common blocks
//FIX!!   doitloadzdhits_(&layer, &wire, &tdc, &time, &charge, &phi, &distance,
//		       &xpos, &ypos, &cdla, &qual, &earlyGap,
//		       &lateGap, &ntdc, &idx);

       // Keep track of this hit's ID
       int hitId = aZDHit->identifier();
       
       // Check that we know what we are doing - idx should be l+1
       if ( idx != l+1 )
       {
	 report(EMERGENCY,kFacilityString) << "ZD Problem with idx" << endl;
	 assert(idx == l+1);
       }

       // Do we have to add to the vector?
       if ( m_ZDhitIdentifiers.size() <= l )
       {
	  const_cast<DoitTrackFinder*>(this)->m_ZDhitIdentifiers.
	     push_back(hitId);
       }
       else
       {
	  const_cast<DoitTrackFinder*>(this)->m_ZDhitIdentifiers[l] = hitId;
       }
     } // for over ZD hits
   }  // if there are ZD hits
}
...
// Extract Si hits
   ...
   // Make a lattice for the seedTrack, CalibratedDRHit connection
   auto_ptr<SeedTrackZDHitLattice> pZDLattice( 
      new SeedTrackZDHitLattice(pSeedTracks.get(), 
				zdHits, 
				SeedTrackZDHitLattice::LNodeMulti) );
   ...
   for ( int i = 1; i <= nTracks; ++ i )
   {
	 ...
	 nHitsOnSVZLayer[dummy3] = 0 ;
      ...
      int nHitsOnZDLayer[7];
      for( int dummy4 = 0 ; dummy4 < 7 ; ++dummy4 )
      {
	 nHitsOnZDLayer[dummy4] = 0 ;
      }
	...
	pSeedTracks->find(i);
      ...
      int smallestZDLayer = 10;
      int largestZDLayer = 0;
	...
	else if ( device == kDoitDeviceZD )
	{
	   //FIX!!
	   if ( diagLevel > 0 )
	      report ( INFO, kFacilityString ) << "ZD Hit on track " << i 
					       << " " 
					       << "Layer = " << layer << " " 
					       << "wire = " << wire << " "
					       << "pull = " << pull << " " 
	  ...
	  if ( m_hitIdentifiers.size() <= (iadr-1) )
	  {
	    report(EMERGENCY, kFacilityString) << "m_ZDhitIdentifier is " 
					       << "too small" << endl;
	    assert(false);
	  }

	  int hitId = m_ZDhitIdentifiers[iadr-1];
	  
	  if ( zdHits[hitId].layer()     != layer ||
	       zdHits[hitId].wireInLyr() != wire      )
	  {
	    report(EMERGENCY, kFacilityString) 
	      << "Internal ZD hit inconsistency! \n" 
	      << "  Have hitId, layer, wire " << hitId 
	      << " " << layer << " " << wire
	      << "\n  Got instead layer, wire "
	      << zdHits[hitId].layer()
	      << " " << zdHits[hitId].wireInLyr()  << endl;
	    
	    assert(zdHits[hitId].layer()     == layer &&
		   zdHits[hitId].wireInLyr() == wire      );
	    ::exit(1);
	  }
	  
	  // Make an iterator pointing to this hit
	  FATable<CalibratedZDHit>::const_iterator zdIter = 
	    zdHits.find(hitId);
	  
	  if ( diagLevel > 0 )
	    report ( INFO, kFacilityString ) 
	      << "Connecting hit (" << layer << ", " << wire << ")"
	      << "[ZD hit id = " << hitId << ", zdIter points to " 
	      << zdIter->identifier() << " ] to seed track " << i << endl;
	  
	  // Connect this hit to the seed track with the lattice

	  // Setup the fit flag
	  SeedZDHitLink::fitFlagEnum fitFlag;
	  switch ( buckcd )
	  {
	    case 0:
	      fitFlag = SeedZDHitLink::kFITTABLE;
	      break;

	    default:
	      fitFlag = SeedZDHitLink::kDO_NOT_FIT;
	  };
	  
	  // Make the link data (use default fitting weight for now)
	  SeedZDHitLink latlink(meas, pull, fitFlag);
	  
	  // Connect - a copy of latlink is made
	  if ( pZDLattice->connect(seedIter, zdIter, latlink) == 0 )
	  {
	    report(EMERGENCY, kFacilityString) 
	      << "ZD wire Lattice connect failed" << endl;
	  }
	  
	   // Update the layer watchers
	  if ( layer < smallestZDLayer ) smallestZDLayer = layer;
	  if ( layer > largestZDLayer  ) largestZDLayer  = layer;

	  // Keep track of how many hits are on this layer
	  nHitsOnZDLayer[layer]++;
	}
...
// getZDLattice 
//
//  Get the lattice connecting seed tracks and ZD hits from the
//  last Doit call
SeedTrackZDHitLattice* 
DoitTrackFinder::getZDLattice() const
{ 
...
void DoitTrackFinder::clearCache() 
{
  ...
  if ( m_doitZDLattice != 0 )
  {
    delete m_doitZDLattice;
    m_doitZDLattice = 0;
  }
...
void DoitTrackFinder::setZDUsage(DABoolean useZD)
{
  m_useZD = useZD;
}
