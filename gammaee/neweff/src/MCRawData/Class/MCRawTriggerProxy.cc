...
// $Id: MCRawTriggerProxy.cc,v 1.28 2004/02/02 15:16:51 vk Exp $
//
// Revision history
//
// $Log: MCRawTriggerProxy.cc,v $
// Revision 1.28  2004/02/02 15:16:51  vk
// Put back ConstantsMapping/CC.h since it's rule of Common.
//
// Revision 1.27  2004/02/01 19:16:10  vk
// Fixed include without package prefix
//
				 ...
				 const DataKey& iKey )
{
   ...
   cout << "MCRawTriggerProxy::faultHandler()" << endl;


   if( m_getRawTrigger.value() )
   {
      cout << "in getRawTrigger.value() == true" << endl;
                                   ...
                                   TriggerData& trigDat)
{
   ...
   cout << "MCRawTriggerProxy::makeTriggerData()" << endl;
                                     ...
                                     TriggerData& trigDat)
{
   ...
   for(int bucket = 0; bucket < 4; bucket++)
   {
      ...
      cout << "MCRawTriggerProxy::makeL1TriggerData() bucket " << bucket
	   << " " << trigDat.getHiTrackCount(bucket) << " HiTracks, "
	   << " " << trigDat.getLoTrackCount(bucket) << " LoTracks, "
	   << " " << trigDat.getCBMdCount(bucket) << " CBMds, "
	   << " " << trigDat.getCBLoCount(bucket) << " CBLos, "
	   << " " << trigDat.getAxTrackCount(bucket) << " AxTracks, "
	   << " " << trigDat.getCBHiOld(bucket) << " CBHiOlds, "
	   << " " << trigDat.getCEHiOld(bucket) << " CEHiOlds" << endl;
                                           ...
                                           TrigCBArray *lumiPtr)
{
   ...
   cout << "MCRawTriggerProxy::makeTrackingTriggerData()" << endl;
   ...
   if (firstTime)
   {
      ...
      { 
	 cout << "yikes!  returning so soon?" << endl;
...
//      UInt32 cellID ( rawDR->rawDaqId(word) );
      ...
      cout << "good add_hit_(" << layer << ", " << wire << ", " << time << ")" << endl;
   ...
   cout << "numAxial: " << numAxial << " and numStereo: " << numStereo << endl;
   ...
   if (useDeadWires)
   {
             ...
             deadWireConst.begin(); itr != itrEnd; ++itr)
      {
         ...
         int time = 0;
	 cout << "dead add_hit_(" << layer << ", " << wire << ", " << time << ")" << endl;
         add_hit_(&layer, &wire, &time );
      }
   }
   ...
   cout << "sun sucks filling stereo arrays... " << endl;
   ...
   for (int layer = 1; layer <= kLayers; ++layer)
   {
           ...
           ++block)
      {
         ...
         {
	    cout << "stereoBlocks.setBlock(" << layer << ", " << block << ", " << kAll << ")" << endl;
   ...
   cout << "axial tracking stuff" << endl;

   for (int cell = 1; cell <= 48; ++cell)
   {
      cout << "axMap[" << cell-1 << "] = " << axMap[cell-1] << endl;
   ...
   cout << "now for some lumi bit things (sun still sucks)" << endl;

   for (bit = 0; bit < kAxCellSize; ++bit)
   {
      cout << "axial[" << bit << "] = " << axial[bit] << endl;
   ...
   {
      cout << "lowPos[" << bit << "] = " << lowPos[bit] << endl;
   ...
   cout << "finally all the tracking information is put into the TriggerData object" << endl;
                                     ...
                                     TrigCBArray *lumiPtr)
{
   ...
   cout << "MCRawTriggerProxy::makeCCTriggerData()" << endl;
   ...
   cout << "How many CcEnergyHits? " << hits.size() << endl;
...
*/
   ...
   for(iHit = hits.begin(); iHit != hits.end(); ++iHit)  // loop over CC hits
   {
      ...
      cout << "cellId " << hit.cellId() << " has erg " << erg << " (in GeV, NOT ERGS!)" << endl;
   ...
   for(Tpro = 1; Tpro <= kNumTpros; ++Tpro)  // loop over Tpro boards
   {
      for(int proc = kNumProcPerTpro; proc > 0; --proc)  // processors
      {
	    ...
	    cout << "Tpro " << Tpro << ", proc " << proc << " has prob "
		 << probH << " " << probM << " " << probL << " for hi, mid, lo shower (respectively)" << endl;
	    ...
	    cout << "random number " << randNum << " so shower is "
		 << (showerType == kHi ? "high" : (showerType == kMd ? "mid" : "low")) << "!" << endl;
   ...
   cout << "then we throw some more random numbers..." << endl;
   ...
   for(MB = 1; MB <= kNumMBs; ++MB)  // motherboard loop
   {
      ...
      for(int DB = DBfirst; DB <= DBlast; ++DB)  // daughterboard loop
      {
         ...
         {
	    cout << "timeBucket[" << MB << "][" << DB << "][" << bucket << "] = "
		 << timeBucket[MB][DB][bucket] << endl;
