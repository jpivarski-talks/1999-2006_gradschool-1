...
// $Id: MCRawRichHitsProxy.cc,v 1.12 2003/01/29 19:31:07 cleo3 Exp $
//
// Revision history
//
// $Log: MCRawRichHitsProxy.cc,v $
// Revision 1.12  2003/01/29 19:31:07  cleo3
// changes needed to compile using g++
//
// Revision 1.11  2002/09/27 20:23:11  bkh
// ZD addition
...
MCRawRichHitsProxy::MCRawRichHitsProxy(
   const Parameter<DABoolean>&   aMakeMCRawRichHits,
   const Parameter<DABoolean>&   aUseFakeMerge ,
   const Parameter<float>    &   aUseNoiseScaleFactor        ) :
   m_RawRichHits ( 0 ) ,
   m_words       (   ),
   m_RICHDBPedestals(),
   m_RICHDBGains(),
   m_averageDBGain(0),
   m_makeMCRawRichHits ( aMakeMCRawRichHits ) ,
   m_useFakeMerge      ( aUseFakeMerge      ) ,
   m_NoiseScaleFactor  ( aUseNoiseScaleFactor )
{
   report( DEBUG, kReport ) 
      << "MakeMCRawRichHits: " << m_makeMCRawRichHits.value() << endl;

   m_words.reserve( 10000 ) ; // allocate reasonable length just once!

   if( m_makeMCRawRichHits.value() ) 
   {
	   ...
	   // initialize ph
	      ...
	      *(pPh++)=0; };
	      ...
	      it!=itEnd; ++it){
           ...
           // add background ?
                rich_add_background_s_( is, im, ph.cArray(),
					FReal( m_NoiseScaleFactor.value() ) ) ;
	   }
