...
// $Id: MCRawHeaderProxy.cc,v 1.8 2003/10/16 21:08:56 vk Exp $
//
// Revision history
//
// $Log: MCRawHeaderProxy.cc,v $
// Revision 1.8  2003/10/16 21:08:56  vk
// Correctly check time stamp, it should be > 1 for MC.
//
				...
				const DataKey& iKey )
{
   ...
   if( iRecord.frame().contains( Stream::kEvent ) &&
       iRecord.frame().record( Stream::kEvent ).syncValue().time()<=1 ) 
   {
