...
C   Use to fit observed line shape: sig(W_CESR)=A*GBWKF(W_CESR) (in MeV^-2)
...
C
	...
	COMMON/CKF/HT(0:MTBIN),BWKF(-MTBIN:MTBIN) ! BW-F_KF convolution
...
C            GAM/(2pi) normalizes Breit-Wigner
	  ELSE
	    SM=WT**2-RM**2
	    BWKF(ITBIN)=0.D0
	    IF(SM.GT.0.D0)BWKF(ITBIN)=2.D0*FKF(SM/S,S)/RM  ! BW ~ delta fn
	  ENDIF
 14	CONTINUE
C
...
C---Don't smear if overflow MKBINs
 12	SM=(HC+RM)**2-RM**2
	GBWKF=0.d0
	IF(SM.GT.0.d0)GBWKF=2.d0*FKF(SM/S,S)/RM     ! ignore GAM, WSPREAD
	RETURN
C
 ...
 20	SUM=0.d0
	DO 21 KBIN=1,MKBIN                     ! loop over array points
	  ...
	  IF(P.LT.PMAX)THEN
	    ...
	    SUM=SUM+DEXP(-P)*FKFT(KBIN)*DWA(KBIN)
	  ENDIF
 21	CONTINUE
