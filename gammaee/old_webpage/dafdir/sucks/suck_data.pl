...
foreach $title ( sort keys %en )
{
    ...
    print "Online Lumi VS. Run:\n";
    foreach $run ( sort keys %{ $online{$title} } )
    { print "{ { $run, $online{$title}{$run}->[0] }, ErrorBar[ $online{$title}{$run}->[1] ] },\n"; }
    print "\n";

    print "Pass1 Lumi VS. Run:\n";
    foreach $run ( sort keys %{ $online{$title} } )
    { print "{ { $run, $pass1{$run}->[0] }, ErrorBar[ $pass1{$run}->[1] ] },\n"
	  if ( defined( $pass1{$run} ) ); }
    print "\n";

    print "Online Lumi VS. Energy:\n";
    foreach $run ( sort keys %{ $online{$title} } )
    { print "{ $en{$title}{$run}, $online{$title}{$run}->[0], $online{$title}{$run}->[1] },\n"; }
    print "\n";

    print "Pass1 Lumi VS. Energy:\n";
    foreach $run ( sort keys %{ $en{$title} } )
    { print "{ $en{$title}{$run}, $pass1{$run}->[0], $pass1{$run}->[1] },\n"
	  if ( defined( $pass1{$run} ) ); }
    print "\n";
