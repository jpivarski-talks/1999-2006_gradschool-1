...
//    joe         7 Jul 2000                Beef up vectorisr hists
//
//------------------------------------------------------------------------
//
//
...
extern "C" {
  ...
  extern float rlu_();
  extern float random_module_flat_(int *);
}

float random_module_flat_(int *){
...
//Define random number fcn used by PYTHIA
float pyr_(){
  return EvtRandom::Flat();
}
...
void runRhoRho(int nevent,EvtGen& myGenerator,HepLorentzVector t_init);
...
void runStdHep(int nevent,EvtGen& myGenerator);
...
int main(int argc, char* argv[]){
  ...
  if (argc==1){
    ...
    EvtTensor4C T=dual(directProd(p,k));
  ...
  if (!strcmp(argv[1],"file")) {
    ...
    runFile(nevent,argv[3],myGenerator,t_init);
  }

  if (!strcmp(argv[1],"filevpho")) {
    int nevent=atoi(argv[2]);
    runFileVpho(nevent,argv[3],myGenerator,t_init);
  }

  if (!strcmp(argv[1],"test1")) {
    int nevent=atoi(argv[2]);
    runTest1(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"rhorho")) {
    int nevent=atoi(argv[2]);
    runRhoRho(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"chi1kstar")) {
    int nevent=atoi(argv[2]);
    runChi1Kstar(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"test2")) {
    int nevent=atoi(argv[2]);
    runTest2(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"omega")) {
    int nevent=atoi(argv[2]);
    runOmega(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"stdhep")) {
    int nevent=atoi(argv[2]);
    runStdHep(nevent,myGenerator);
  }
  ...
  if (!strcmp(argv[1],"photos")) {
    int nevent=atoi(argv[2]);
    runPhotos(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"trackmult")) {
    int nevent=atoi(argv[2]);
    runTrackMult(nevent,myGenerator,t_init);
  }
  ...
  if (!strcmp(argv[1],"generic")) {
    ...
    runGeneric(nevent,myGenerator,t_init,listfile);
  }

  if (!strcmp(argv[1],"finalstates")) {
    int nevent=atoi(argv[2]);
    runFinalStates(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"kstarnunu")) {
    int nevent=nevent=atoi(argv[2]);
    runKstarnunu(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"bsmix")) {
    int nevent=atoi(argv[2]);
    runBsmix(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"BtoXsgamma")) {
    int nevent=atoi(argv[2]);
    runBtoXsgamma(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"BtoK1273gamma")) {
    int nevent=atoi(argv[2]);
    runBtoK1273gamma(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"pi0dalitz")) {
    int nevent=atoi(argv[2]);
    runPi0Dalitz(nevent,myGenerator,t_init);
  }

  if (!strcmp(argv[1],"ddalitz")) {
    int nevent=atoi(argv[2]);
    runDDalitz(nevent,myGenerator,t_init);
  }


  if (!strcmp(argv[1],"kstarll")) {
    int nevent=atoi(argv[2]);
    runKstarll(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"vectorisr")) {
    int nevent=atoi(argv[2]);
    runVectorIsr(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"bsquark")) {
    int nevent=atoi(argv[2]);
    runBsquark(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"k3gamma")) {
    int nevent=atoi(argv[2]);
    runK3gamma(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"lambda")) {
    int nevent=atoi(argv[2]);
    runLambda(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"tautaupipi")) {
    int nevent=atoi(argv[2]);
    runTauTauPiPi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"tautauee")) {
    int nevent=atoi(argv[2]);
    runTauTauEE(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"tautau2pi2pi")) {
    int nevent=atoi(argv[2]);
    runTauTau2Pi2Pi(nevent, myGenerator, t_init);
  }
  if (!strcmp(argv[1],"tautau3pi3pi")) {
    int nevent=atoi(argv[2]);
    runTauTau3Pi3Pi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"jpsikstar")) {
    int nevent=atoi(argv[2]);
    runJPsiKstar(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"svvcplh")) {
    int nevent=atoi(argv[2]);
    runSVVCPLH(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"svscplh")) {
    int nevent=atoi(argv[2]);
    runSVSCPLH(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"ssdcp")) {
    int nevent=atoi(argv[2]);
    runSSDCP(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"kstarstargamma")) {
    int nevent=atoi(argv[2]);
    runKstarstargamma(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"dstarpi")) {
    int nevent=atoi(argv[2]);
    runDSTARPI(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"etacphiphi")) {
    int nevent=atoi(argv[2]);
    runETACPHIPHI(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"vvpipi")) {
    int nevent=atoi(argv[2]);
    runVVPiPi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"svvhelamp")) {
    int nevent=atoi(argv[2]);
    runSVVHelAmp(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"partwave")) {
    int nevent=atoi(argv[2]);
    runPartWave(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"partwave2")) {
    int nevent=atoi(argv[2]);
    runPartWave2(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"twobody")) {
    int nevent=atoi(argv[2]);
    runTwoBody(nevent, myGenerator, t_init, argv[3]);
  }

  if (!strcmp(argv[1],"pipipi")) {
    int nevent=atoi(argv[2]);
    runPiPiPi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"bhadronic")) {
    int nevent=atoi(argv[2]);
    runBHadronic(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"singleb")) {
    int nevent=atoi(argv[2]);
    runSingleB(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"pipi")) {
    int nevent=atoi(argv[2]);
    runPiPi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"pipipipi")) {
    int nevent=atoi(argv[2]);
    runPiPiPiPi(nevent, myGenerator, t_init);
  }
     
  if (!strcmp(argv[1],"a2pi")) {
    int nevent=atoi(argv[2]);
    runA2Pi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"helamp")) {
    int nevent=atoi(argv[2]);
    runHelAmp(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"helamp2")) {
    int nevent=atoi(argv[2]);
    runHelAmp2(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"d2pi")) {
    int nevent=atoi(argv[2]);
    runD2Pi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"a1pi")) {
    int nevent=atoi(argv[2]);
    runA1Pi(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"cptest")) {
    int nevent=atoi(argv[2]);
    runCPTest(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"pipicpt")) {
    int nevent=atoi(argv[2]);
    runPiPiCPT(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"jpsiks")) {
    int nevent=atoi(argv[2]);
    runJpsiKs(nevent, myGenerator, t_init);
  }
  
  if (!strcmp(argv[1],"jetsettab3")) {
    int nevent=atoi(argv[2]);
    runJetsetTab3(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"dump")) {
    int nevent=atoi(argv[2]);
    runDump(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"random")) {
    int nevent=atoi(argv[2]);
    runRandom(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"uclaII")) {
    int nevent=atoi(argv[2]);
    runUCLAII(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"genericcont")) {
    int nevent=atoi(argv[2]);
    runGenericCont(nevent, myGenerator, t_init);
  }


  if (!strcmp(argv[1],"d1")) {
    int nevent=atoi(argv[2]);
    runD1(nevent, myGenerator, t_init);
  }
  
  if (!strcmp(argv[1],"mix")) {
    int nevent=atoi(argv[2]);
    runMix(nevent, myGenerator, t_init);
  }

  if (!strcmp(argv[1],"bmix")) {
    int nevent=atoi(argv[2]);
    runBMix(nevent, myGenerator, t_init);
  }
  
  if (!strcmp(argv[1],"semic")) {
    int nevent=atoi(argv[2]);
    runSemic(nevent,myGenerator,t_init);
  }

 if (!strcmp(argv[1],"etaprk")) {
     int nevent=atoi(argv[2]);
     runEtaPrK(nevent,myGenerator,t_init);
   }
 if (!strcmp(argv[1],"ddk")) {
     int nevent=atoi(argv[2]);
     runDDK(nevent,myGenerator,t_init);
   }
 ...
 if (!strcmp(argv[1],"checkmass")) {
   ...
   if (!strcmp(argv[1],"jpsipolarization")) {
     int nevent=atoi(argv[2]);
     runJpsiPolarization(nevent,myGenerator,t_init);
   }
...
void runJpsiPolarization(int nevent, EvtGen &myGenerator, 
   ...
   myGenerator.readUDecay("GENERIC.DEC");
   myGenerator.readUDecay("JPSITOLL.DEC");
...
void runEtaPrK(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
   ...
   do{
     ...
     root_part->setVectorSpinDensity();      
     myGenerator.generateEvent(root_part,t_init);
     EvtParticle *pb1=root_part->getDaug(0);
     EvtParticle *pb2=root_part->getDaug(1);

     if ( pb1->getNDaug()==2 ) {
       if ( pb1->getDaug(0)->getId() == KP && pb1->getDaug(1)->getId() == ETAPR ) kp+=1;
       if ( pb1->getDaug(1)->getId() == KP && pb1->getDaug(0)->getId() == ETAPR ) kp+=1;
       if ( pb1->getDaug(0)->getId() == KM && pb1->getDaug(1)->getId() == ETAPR ) km+=1;
       if ( pb1->getDaug(1)->getId() == KM && pb1->getDaug(0)->getId() == ETAPR ) km+=1;
     }
                                                                             
     if ( pb2->getNDaug()==2 ) {
       if ( pb2->getDaug(0)->getId() == KP && pb2->getDaug(1)->getId() == ETAPR ) kp+=1;
       if ( pb2->getDaug(1)->getId() == KP && pb2->getDaug(0)->getId() == ETAPR ) kp+=1;
       if ( pb2->getDaug(0)->getId() == KM && pb2->getDaug(1)->getId() == ETAPR ) km+=1;
       if ( pb2->getDaug(1)->getId() == KM && pb2->getDaug(0)->getId() == ETAPR ) km+=1;
     }
     root_part->deleteTree();  
   }while (count++<nevent);

   report(INFO,"EvtGen") << "etapr kp="<<kp<<" etapr km="<<km<<std::endl;
 }
...
void runPi0Dalitz(int nevent, EvtGen &myGenerator, 
  ...
  int count;
    ...
    myGenerator.readUDecay("PI0DALITZ.DEC");
...
void runRhoRho(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  //  TFile *file=new TFile("test1.root", "RECREATE");
  static EvtId UPS4=EvtPDL::getId(std::string("Upsilon(4S)"));
  static EvtId RHO0=EvtPDL::getId(std::string("rho0"));
  static EvtId RHOP=EvtPDL::getId(std::string("rho+"));
  static EvtId RHOM=EvtPDL::getId(std::string("rho-"));
  static EvtIdSet theRhos(RHO0,RHOP,RHOM);
  
  //  int first=0;
  //  char **second;
  //  TApplication *theApp = new TApplication("App", &first, second);
  TFile *file = new TFile("rhorho.root", "RECREATE", "Example");

  TH2F* hist1 = 
    new TH2F("cvc","cos1 vs cos2",50,-1.,1.,50,-1.0,1.0); 
  ...
  EvtParticle *root_part;
  EvtVectorParticle *vector_part;
  EvtVector4R p4rho1,p4rho2,p4pi1,p4pi2,p4b;
  char udecay_name[100];
  
  strcpy(udecay_name,"RHORHO.DEC");
  
  //    EvtGen myGenerator(decay_name,pdttable_name,myRandomEngine);
  
  myGenerator.readUDecay(udecay_name);
  double costhetaV1,costhetaV2;
  ...
  do{
    ...
    //std::cout <<stdhep<<std::endl;

    int whichB=-1;
    if ( root_part->getDaug(0)->getNDaug()==2 ) {
      if ( theRhos.contains(root_part->getDaug(0)->getDaug(0)->getId()) &&
	   theRhos.contains(root_part->getDaug(0)->getDaug(1)->getId()) ) whichB=0;
    }
    if ( root_part->getDaug(1)->getNDaug()==2 ) {
      if ( theRhos.contains(root_part->getDaug(1)->getDaug(0)->getId()) &&
	   theRhos.contains(root_part->getDaug(1)->getDaug(1)->getId()) ) whichB=1;
    }

    if ( whichB>-1) {
      
      p4b=root_part->getDaug(whichB)->getP4Lab();
      p4rho1=root_part->getDaug(whichB)->getDaug(0)->getP4Lab();
      p4rho2=root_part->getDaug(whichB)->getDaug(1)->getP4Lab();
      
      p4pi1=root_part->getDaug(whichB)->getDaug(0)->getDaug(0)->getP4Lab();
      p4pi2=root_part->getDaug(whichB)->getDaug(1)->getDaug(0)->getP4Lab();
      
      costhetaV1=EvtDecayAngle(p4b,p4rho1,p4pi1);
      costhetaV2=EvtDecayAngle(p4b,p4rho2,p4pi2);
      hist1->Fill(costhetaV2,costhetaV1);
    }
    else{
      EvtStdHep stdhep;
      stdhep.init();
...
void runDDK(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"GENERIC.DEC");
...
void runChi1Kstar(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"CHI1KSTAR.DEC");
...
void runStdHep(int nevent, EvtGen &myGenerator) {
  ...
  static EvtId UPS4=EvtPDL::getId(std::string("vpho"));
    
  int stdhepid=EvtPDL::getStdHep(UPS4);
  
  EvtStdHep evtstdhep;
  
  EvtVector4R d(0.0,0.0,0.0,0.0);
  
  do{
    
    EvtVector4R p_init(10.55,0.0,0.0,0.0);

    myGenerator.generateDecay(stdhepid,p_init,d,&evtstdhep);
    
    //report(INFO,"EvtGen") << evtstdhep << std::endl;
...
void runFinalStates(int nevent, EvtGen &myGenerator, 
  ...
  EvtParser parser;
  parser.Read(std::string("finalstates.list"));
...
void runKstarnunu(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"KSTARNUNU.DEC");
  //EvtGen myGenerator(decay_name,pdttable_name,myRandomEngine);
  myGenerator.readUDecay(udecay_name);
...
void runKstarll(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  TFile *file=new TFile("kstarll.root", "RECREATE");
  
  TH2F* dalitz  = new TH2F("h1","q^2! vs Elep",
					  70,0.0,3.5,60,0.0,30.0); 
  
  
  TH1F* ctl  = new TH1F("h2","ctl",50,-1.0,1.0); 
  
  
  TH1F* q2  = new TH1F("h3","q2",50,0.0,25.0); 
  
  TH1F* q2small = new TH1F("h4","q2 (low)",
					50,0.0,1.0); 
  TH1F* q2lowlow = new TH1F("h5","q2 (lowlow)",
					   50,0.0,0.00001); 

  TH1F* phi  = new TH1F("h6","phi",50,-EvtConst::pi,EvtConst::pi); 

  TH1F* chi  = new TH1F("h7","chi",50,0.0,EvtConst::twoPi); 

  TH1F* chictl  = new TH1F("h8","chictl",50,0.0,EvtConst::twoPi); 
  ...
  char udecay_name[100];
  strcpy(udecay_name,"KSTARLL.DEC");
  //EvtGen myGenerator(decay_name,pdttable_name,myRandomEngine);
  ...
  do{
    ...
    q2->Fill( (l1+l2).mass2() );
    q2small->Fill( (l1+l2).mass2() );
    q2lowlow->Fill( (l1+l2).mass2() );
    
    ctl->Fill(EvtDecayAngle((l1+l2+kstar),(l1+l2),l1));
    
    dalitz->Fill(l1.get(0),(l1+l2).mass2(),1.0);
    
    root_part->deleteTree();  

    phi->Fill(atan2(l1.get(1),l1.get(2)));

    chi->Fill(EvtDecayAngleChi(b,k,pi,l1,l2));

    if (EvtDecayAngle((l1+l2+kstar),(l1+l2),l1)>0){
      chictl->Fill(EvtDecayAngleChi(b,k,pi,l1,l2));
    }
...
void runVectorIsr(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"VECTORISR.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runBsquark(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"BSQUARK.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runK3gamma(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"K3GAMMA.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runTauTauPiPi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"TAUTAUPIPI.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runTauTauEE(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"TAUTAUEE.DEC");
  myGenerator.readUDecay(udecay_name);
  ...
  do{
    ...
    EvtParticle *root_part=EvtParticleFactory::particleFactory(UPS4, p_init);
    root_part->setVectorSpinDensity();      
...
void runTauTau2Pi2Pi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"TAUTAU2PI2PI.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runTauTau3Pi3Pi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  strcpy(udecay_name,"TAUTAU3PI3PI.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runJPsiKstar(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  if (nevent==0) strcpy(udecay_name,"JPSIKSTAR.DEC");
  if (nevent==1) strcpy(udecay_name,"JPSIKSTAR1.DEC");
  if (nevent==2) strcpy(udecay_name,"JPSIKSTAR2.DEC");
  if (nevent==3) strcpy(udecay_name,"JPSIKSTAR3.DEC");
  if (nevent==4) strcpy(udecay_name,"JPSIKSTAR4.DEC");
...
void runSVVCPLH(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"SVVCPLH.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runSVSCPLH(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"SVSCPLH.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runKstarstargamma(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  myGenerator.readUDecay("KSTARSTARGAMMA.DEC");
...
void runDSTARPI(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  myGenerator.readUDecay("DSTARPI.DEC");
...
void runETACPHIPHI(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"ETACPHIPHI.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runSVVHelAmp(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"SVVHELAMP.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runPartWave(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"PARTWAVE.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runPartWave2(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"PARTWAVE2.DEC");
  myGenerator.readUDecay(udecay_name);
...
void runBtoXsgamma(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"BTOXSGAMMA.DEC");
...
// debug stuff:      report(INFO,"EvtGen") << "bnum=" << bnum << " pengcount=" << pengcount << " bId=" << bId << " Bmulti=" << Bmulti << " XsId=" << XId << " gId=" << gId << std::endl;
	...
	if(abs(EvtPDL::getStdHep(XsDaugId))==321||EvtPDL::getStdHep(XsDaugId)==310||EvtPDL::getStdHep(XsDaugId)==111||abs(EvtPDL::getStdHep(XsDaugId))==211||Daumulti==0){
	  nTot++;
	  EvtParticle *XsDaugNephew = Xs->getDaug(i);
...
void runBtoK1273gamma(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
  ...
  char udecay_name[100];
  strcpy(udecay_name,"BTOK1273GAMMA.DEC");
...
void runBMix(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("BMIX.DEC");
...
void runPiPiPi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("PIPIPI.DEC");
...
void runBHadronic(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("BHADRONIC.DEC");
...
void runPiPiPiPi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("PIPIPIPI.DEC");
...
void runA2Pi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("A2PI.DEC");
...
void runHelAmp(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("HELAMP.DEC");
...
void runHelAmp2(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("HELAMP2.DEC");
...
void runD2Pi(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("D2PI.DEC");
...
void runPiPiCPT(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("PIPICPT.DEC");
...
void runJpsiKs(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("JPSIKS.DEC");
...
void runJetsetTab3(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("JETSETETAB3.DEC");
    ...
    do{
      ...
      //root_part->makeStdHep(stdhep,secondary,stable_list);
...
void runDump(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("DUMP.DEC");
...
void runRandom(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    report(INFO,"EvtGen") << "The random test is broken!"<<std::endl;
    ...
    std::ofstream outmix;
    outmix.open("generic.dat");

    EvtParticle *root_part;
    EvtVectorParticle *vector_part;

    myGenerator.readUDecay("");

    int iseed1,iseed2,iseq;
    iseed1=123457;
    iseed2=376983;
    iseq=1;
    ...
    for(itry=0;itry<2;itry++){
      ...
      report(INFO,"EvtGen") << "In Random:"<<EvtRandom::Flat(0.0,1.0)<<std::endl;
	...
	root_part->deleteTree();  
      ...
      report(INFO,"EvtGen") << "Out Random:"<<EvtRandom::Flat(0.0,1.0)<<std::endl;
    }
...
void runUCLAII(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    std::ofstream outmix;
    outmix.open("uclaII.dat");

    EvtParticle *root_part;
    EvtVectorParticle *vector_part;
    EvtParticle *p;

    myGenerator.readUDecay("uclaII.dec");
    ...
    do{
      ...
      do{
        ...
        outmix << EvtPDL::getStdHep(p->getId())<<" "<< p->getP4Lab().d3mag() << std::endl;
...
void runMix(int nevent, EvtGen &myGenerator, HepLorentzVector t_init) {
    ...
    myGenerator.readUDecay("MIX.DEC");
