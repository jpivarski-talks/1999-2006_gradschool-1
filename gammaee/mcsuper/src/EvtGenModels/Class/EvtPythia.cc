...
void EvtPythia::init(){
  ...
  if (getParentId().isAlias()){
    ...
    report(ERROR,"EvtGen") << "EvtPythia finds that you are decaying the"<<std::endl
                           << " aliased particle "
...
void EvtPythia::WritePythiaEntryHeader(std::ofstream &outdec, int lundkc,
  ...
  outdec << setiosflags(std::ios::left)
	 << sname;
  ...
  outdec << resetiosflags(std::ios::left);
