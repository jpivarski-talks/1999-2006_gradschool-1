...
//      $Id: EvtPto3P.cc,v 1.2 2004/08/10 13:53:07 ryd Exp $
// 
// Environment:
...
EvtAmpFactory<EvtDalitz2D>* EvtPto3P::createFactory(const EvtMultiChannelParser& parser)
{
  // Compute the interval size

  EvtDalitzPlot plot = dp();
  EvtDalitz2D ival(plot);
  ival.compute_size(10000);

  EvtAmpFactory<EvtDalitz2D>* fact = new EvtPto3PAmpFactory(plot);
  fact->build(ival,parser);
  return fact;
}
