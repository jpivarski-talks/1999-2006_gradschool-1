...
void EvtJetSet::getName(std::string& model_name){
...
void EvtJetSet::decay( EvtParticle *p){
  ...
  do{
    //report(INFO,"EvtGen") << "calling jetset " << ip<< " " << mp <<std::endl;
    jetset1_(&ip,&mp,&ndaugjs,kf,km,px,py,pz,e);
    

    numstable=0;
    numparton=0;
    //report(INFO,"EvtGen") << "found some daughters " << ndaugjs << std::endl;
    for(i=0;i<ndaugjs;i++){
  ...
  if (count>9999) {
    ...
    report(INFO,"EvtGen") << "Parent:"<<EvtPDL::name(getParentId()).c_str()<<std::endl;
    for(i=0;i<numstable;i++){
      report(INFO,"EvtGen") << "Daug("<<i<<")"<<EvtPDL::name(evtnumstable[i]).c_str()<<std::endl;
    }
  ...
  if (numparton==0){
    ...
    if ( ndaugFound == 0 ) {
      ...
      report(ERROR,"EvtGen") << EvtPDL::name(p->getId()).c_str() << " " << p->mass()<<std::endl;
      assert(0);
    }
...
void EvtJetSet::WriteJetSetEntryHeader(std::ofstream &outdec, int lundkc,
  ...
  outdec << std::setw(5) << lundkc << "  ";
  outdec.width(namelength);
  outdec << setiosflags(std::ios::left) << sname << resetiosflags(std::ios::left);
  ...
  outdec.width(12);
  if (std::fabs(width)<0.0000000001) {
    outdec << 0.0 ;
  }
  else{
    outdec << maxwidth;
  }
  outdec << std::setw(14) << ctau;
  outdec.width(3);
  if (evtnum.getId()>=0) {
...
void EvtJetSet::WriteJetSetParticle(std::ofstream &outdec,EvtId ipar,
  ...
  for(ijetset=0;ijetset<njetsetdecays;ijetset++){
	...
	for(i=0;i<jetsetdecays[ijetset]->getNDaug();i++){
	  if (lucomp_(&daugs[i])==0) {
	    ...
	    report(ERROR,"EvtGen") << "JetSet (lucomp) does not "
				  << "know the particle:"<<
	      EvtPDL::name(jetsetdecays[ijetset]->getDaugs()[i]).c_str()<<std::endl;
	  }
	}
	...
	if (lucomp_(&istdheppar)==0){
	  ...
	  report(ERROR,"EvtGen") << "JetSet (lucomp) does not "
	          << "know the particle:"<<
	      EvtPDL::name(ipar).c_str()<<std::endl;
	}
